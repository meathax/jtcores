# K054156/K054157 register notes derived from the netlist-conversion reference (recovered)

Provenance note (2026-09-03, B2 pass): this file preserves the content that
used to live at `modules/jt05415x/doc/register_map.md` before this session's
B2 pass rewrote that file to document the *currently live* register file
(`jt054157_mmr.v`, generated from `modules/jt05415x/cfg/mmr.yaml`). Recovered
from git history (`git show HEAD:modules/jt05415x/doc/register_map.md`,
commit `bf4a22c`, dated 2026-09-01, message "cps3: standard scene simulation
path" -- an unrelated bulk commit that appears to have swept this file in
incidentally). Preserved rather than discarded because it is independently
useful evidence, sourced from `modules/jt05415x/doc/054156/jt054156.v` and
`jt054156_all.v` (a converted netlist/gate-level reference, i.e. tier-1
silicon evidence, distinct from the Furrtek README used in the new
`register_map.md`), even though its own header claims to describe "the
current HDL extraction" in `modules/jt05415x/hdl/jt054156.v`/`jt054157.v` --
that is no longer accurate: there is no `hdl/jt054156.v` in the current tree
(the 054156 register file is now `jt054156_mmr.v`, generated), and none of
the function/signal names below (`reg0_db`, `jt054156_page01_low_reg_wr_decode`,
etc.) appear anywhere under `hdl/`; they only exist in the `doc/054156/`
reference conversion. Treat the *per-bit meanings* below as tier-1 evidence
about the physical chip; treat the *file/signal names* as pointers into the
`doc/054156/` reference, not the live implementation.

Cross-reference to the new `register_map.md`: the "Second Bank, K054157"
table below independently corroborates two of that document's B2/B3
findings from a different evidence source (silicon netlist conversion,
rather than the Furrtek README + MAME trace):

- `0x02` (`a/b/c/d_hofs_flip`): here described as "Per-layer H offset flip
  enables for HOFSA, HOFSB, HOFSC, and HOFSD" -- agrees with the Furrtek
  README's "Enable tile X flips" reading and with `register_map.md`'s
  decision to wire these as flip enables, not as an offset-sign selector.
  Two independent tier-1 sources now agree.
- `0x06` bits 6-7 (`col_src0/col_src1`): here described as "color/attribute
  column source selection" -- a *more specific* reading than the README's
  "choice of bits in VRAM attribute for X/Y tile flip", and one that reads
  much closer to being the actual B3 candidate (a source-select mux for a
  colour/attribute column, i.e. plausibly the FPAL4/DFI8 5th-palette-bit
  source). This does not by itself justify changing the RTL (the bits are
  still observed constant at `11` for the whole traced ROM run, so no
  polarity/behaviour can be confirmed empirically), but it raises B3's
  priority for a future pass with either a title that toggles these bits or
  a direct read of `doc/054156/jt054156.v`'s gate-level definition of
  `jt054157_page02_cpu_entry` to see exactly what `reg6_d6`/`reg6_d7` drive.

---

<!-- original content below, verbatim except for this note -->
